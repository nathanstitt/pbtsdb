...
describe('optimistic move snap-back via stale query result (on-demand)', () => {
    ...
    it('holds the move when a same-timestamp stale read lands while it is still in flight', async () => {
        // A stale read that returns the genuine pre-move row while the move is in
        // flight: synced still holds that same row, so its `updated` is EQUAL and the
        // timestamp arm cannot tell it apart. The optimistic-pending arm is what holds
        // the move — a read cannot be newer than a mutation still in flight — and it
        // must hold across the settle, until the deferred write-back lands.
        const seed = await seedBook(SOURCE_GENRE)
