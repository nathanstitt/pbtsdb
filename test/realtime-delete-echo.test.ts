...
describe("realtime delete echo idempotency", () => {
    ...
    const syncedHas = (collection: unknown, id: string) =>
        (collection as { _state: { syncedData: { has: (k: string) => boolean } } })._state.syncedData.has(id);
    ...
    it("on-demand: delete echo for an already-pruned key is swallowed", async () => {
        const collection = createCollectionFactory(queryClient).create("books", { syncMode: "on-demand" });
    ...
    it("on-demand: real delete echo for a pruned key does not surface an uncaught error", async () => {
        ...
        expect(captured.filter((e) => /Delete operation: Item with key/i.test(e.message))).toHaveLength(0);
