...
type OrderByExpression = Parameters<typeof parseOrderByExpression>[0]
 ...
 */
export function convertToPocketBaseFilter(where: WhereExpression): PocketBaseFilterString {
    if (!where) return ''
            ...
            in: (field, values) => {
                ...
                if (Array.isArray(values)) {
                    ...
                    const conditions = values.map(
                        v => `${fieldPath} ?= ${formatValueForPocketBase(v)}`
                    )
 ...
 */
export function convertToPocketBaseSort(orderBy: OrderByExpression): PocketBaseSortString {
    if (!orderBy) return ''
